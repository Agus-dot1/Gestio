...
export function setupClientNotificationScheduler(getWindow: () => any, interval?: number) {
  ...
  function emitBatchOrSingle(payloads: any[]) {
    const win = getWindow()
    if (!win || payloads.length === 0) return
    ...
    if (payloads.length >= batchThreshold) {
      ...
      for (const p of payloads) {
        win.webContents.send(IPC_NOTIFICATIONS.event, p)
      }
    }
  }
    ...
    try {
      ...
      if (overdue.length > 0) {
        ...
        for (const inst of overdue) {
          try {
            if (!inst?.id || !(inst as any).customer_name || !inst.balance || inst.balance <= 0) continue
            const customer = (inst as any).customer_name
            ...
            if (!existsActive && !existsToday) {
              if (!(suppressIfMuted && isMuted)) {
                const nid = notificationOperations.create(msg, 'alert', key)
...
export function checkLowStockAfterSaleHook(saleData: any, getWindow: () => any) {
  try {
      ...
      for (const item of saleData.items) {
        try {
          ...
          if (p && typeof p.stock === 'number' && p.stock <= 1) {
            const priceStr = (() => { try { return new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 }).format(p.price) } catch { return `$${p.price}` } })()
            ...
            if (shouldNotify) {
              ...
              if (!(suppressIfMuted && isMuted)) {
                ...
                if (win) {
                  ...
                  win.webContents.send(IPC_NOTIFICATIONS.event, { id: nid, message: msg, type: 'attention', meta: { message_key: key, stockStatus: `${p.stock} unidad${p.stock === 1 ? '' : 'es'}`, productId: p.id, productName: p.name, productPrice: p.price, productCategory: p.category || 'Sin categoría', currentStock: p.stock, ...(createdAtNew ? { created_at: createdAtNew } : {}) } })
                }
              }
            }
          }
    ...
    if (!existsActive && !existsToday) {
      ...
      if (!(suppressIfMuted && isMuted)) {
        ...
        if (win) {
          const latest = notificationOperations.getLatestByKey(key)
          const createdAt = latest?.created_at
          win.webContents.send(IPC_NOTIFICATIONS.event, { id: nid, message: msg, type: 'alert', meta: { message_key: key, customerName, due_at: new Date(inst.due_date).toISOString(), amount: inst.balance, ...(createdAt ? { created_at: createdAt } : {}) } })
        }
      }
    }
  } catch { }
